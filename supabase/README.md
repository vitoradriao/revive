...
O script insere a [fixture legada](verification/legacy_fixture.sql) entre o baseline e as migrations. A [checagem após upgrade](verification/assert_synthetic_upgrade.sql) valida identificadores, relações, defaults e a cascata de exclusão de conta dentro de uma transação revertida. Ele nunca toca no projeto existente.
